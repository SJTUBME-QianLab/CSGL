...
class GetData:
    def __init__(self, cov_kind, y_kind, n):
        ...
        for i in range(p):
            for j in range(i):
                if self.cov_kind == 'Gaussian':
                    Sig[i, j] = np.exp(- abs(loc[i] - loc[j]) / (2 * ss ** 2))
                elif self.cov_kind == 'Laplacian':
                    Sig[i, j] = np.power(rho, - abs(loc[i] - loc[j]) / (2 * ss))
                Sig[j, i] = Sig[i, j]
    ...
    def get_data(self, all_info):
        ...
        betag = []
        for ddi in all_info:
            cov = self.get_L(ddi['coordinate'], rho=None)
